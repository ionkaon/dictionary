...
文件 [`字表.tsv`](字表.tsv) 是完整的字表，爲了製作 [寧波話吳語拼音輸入方案](https://github.com/NGLI/rime-wugniu_gninpou) 而作。該字表現階段包含寧波城區、鎮海、北侖和舟山定海四幾個地方的口音。後續可能會加入更多甬江小片各地的口音作爲對照。
...
以江苏教育版《寧波方言詞典》为基础，综合了《宁波市志·方言》。相比江苏教育版的《寧波方言詞典》，区分了\[ko\]、\[kuo\]及\[kʰo\]、\[kʰuo\]。另外还单独列出了清鼻音边音声母。

声母 \[ɦ\] 后面接韵母 i 或 i 介音时，拼作 yi 或 y- 。后面接韵母 iu 或 iu 介音时，拼作 yu-。后面接韵母 u 或 u 介音时，拼作 wu 或 w-。其余情形拼写为 gh。

零声母开头有轻微的喉塞音 \[ʔ\]，列举声母时作 \[∅\]，搭配韵母时不写。

\[ʏ\] 韵对应 ieu、oe 两种拼音。在声母 t\[t\]、l\[l\]、lh\[ʔl\] 之后拼作ieu，在声母 ts\[ts\]、tsh\[tsʰ\]、dz\[dz\]、s\[s\]、z\[z\] 之后拼作 oe。
...
1. 城區讀音了參照多本資料，對於同一個字的單字調，幾種資料有時會有出入，此時以湯版《寧波方言詞典》的記錄爲準。
2. 對於城區口音，若某字在資料中的兩個讀音符合吳拼教程 [新老差異](https://ionkaon.github.io/phin-in-tutorial/内部差异/新老差异.html) 一節中提到的第 3 點到第 12 點。字表內只記錄其中的老派讀音。
3. 寧波城區老派口音僅有 7 個單字調，其中陽上併入了陽去。但在兩字組的前字位置，陽上陽去有所不同。陽上變爲 23，陽去變爲 22。字表中依據變調的不同將兩者分別標爲 4 和 6。注意這 **並不** 表示兩者的單字調有區別。對於單字調爲陽上陽去，且不出現在前字位置的那些字，字表中依照該字在中古的調類標註。這可能會導致一些錯誤，請使用者自行考量。
